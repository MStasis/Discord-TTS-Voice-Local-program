...
Voiceboard는 선택한 오디오 출력 장치로 TTS와 사운드 파일을 재생합니다. Discord에서는 같은 가상 오디오 케이블의 입력 쪽을 마이크로 선택하면 됩니다.

예시:

- Voiceboard 출력 대상: `CABLE Input` 또는 가상 믹서의 재생 장치
- Discord 입력 장치: `CABLE Output` 또는 가상 믹서의 녹음 장치

VB-CABLE, VoiceMeeter, SteelSeries Sonar 같은 가상 오디오 장치가 필요합니다.
...
- `dist/Voiceboard-Setup-0.1.0-x64.exe`: 설치 파일
- `dist/Voiceboard-Portable-0.1.0-x64.exe`: 무설치 실행 파일
